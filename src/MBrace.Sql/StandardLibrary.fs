...
module StdLib =
    ...
    let rec private shrinkAst (term:TermEx) : TermEx =
        match term with
    ...
    let compileSqlAst (term:TermEx) =
        ...
        let funcQuotation = Quotations.Expr.Cast<Map<string, SqlType> -> SqlType>(Quotations.Expr.Lambda(functionParameter, lambdaBody))
        QuotationCompiler.QuotationCompiler.ToFunc funcQuotation ()
